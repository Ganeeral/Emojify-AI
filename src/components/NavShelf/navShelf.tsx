...
const NavShelf: React.FC<NavShelfProps> = ({ isScrolled, activeSection }) => {
  return (
    <div
      className={`flex z-[999] top-8 fixed ${
        isScrolled ? "opacity-100" : "opacity-0 pointer-events-none"
      } left-1/2 transform -translate-x-1/2 bg-[#464344] px-8 py-3 gap-x-8 items-center rounded-[99px] justify-center transition-opacity duration-500`}
    >
      {data.map((item) => (
